...
const App: React.FC<IAppProps> = ({}) => {
    ...
    useEffect( () => {
      const checkLoggedIn = async () => {
        ...
        const tokenResponse = await Axios.post("//localhost:8174/api/checkToken", null,  {headers: {"x-auth-token": token}});
  
        if(tokenResponse.data){
          const userRes = await Axios.get("//localhost:8174/api/loginwithjwt", {headers: {"x-auth-token": token}})
